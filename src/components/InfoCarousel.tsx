    ...
    const features: FeatureProps[] = [
        {
            ...
            list: ['Condensan la inforación centrándose en lo más importante (y preguntable).',
        ...
        },
        {
            ...
            list: ["Marco jurídico completo, señalando en todo momento la fuente de información. ",
    ...
    return <Carousel opts={{
        ...
        <CarouselContent >
            {features.map(({ image, title, description, list }: FeatureProps) => (
                ...
                <CarouselItem key={title} className="lg:basis-1/1"> <Card
                    key={title}
                ...
                >
                    ...
                    <div className="w-4/5 flex flex-col gap-2">
                        ...
                        <CardContent>{description}
                            {list && <ul className="list-disc list-inside">{list.map(item => <li key={item}>{item}</li>)}</ul>}
